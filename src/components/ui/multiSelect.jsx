...
export const MultiSelect = React.forwardRef(
    ...
    // eslint-disable-next-line react-hooks/exhaustive-deps
		...
		const toggleAll = () => {
			...
			const allOptions = options.flatMap((opt) => (opt.options ? opt.options : opt));
					...
					>
						{selectedValues.length > 0 ? (
							<div className="flex justify-between items-center w-full">
								...
								>
									{selectedValues.slice(0, maxCount).map((value) => {
										// Cari label dari options (flat search)
							...
							<div className="flex items-center justify-between w-full mx-auto">
								<span className="text-sm text-muted-foreground mx-3 normal-case">
									{placeholder}
								</span>
						...
						<CommandList>
							...
							<CommandGroup>
								...
								{options.map((optionOrGroup) => {
									// Cek apakah ini group atau single option
				...
				{animation > 0 && selectedValues.length > 0 && (
					<WandSparkles
						className={cn(
							...
							animationConfig?.badgeAnimation === "pulse" ? "animate-pulse" : ""
						)}
					/>
				)}
			</Popover>
